...
class Application_Window():
    ...
    def __init__(self, program_root):
        ...
        self._config_file = conf.Config_File(self._program_config_root + \
                                                    os.sep + "main.config")
        ...
        if instances_running > 1:
            ...
            dialog = gtk.MessageDialog(self.window,
                ...
                ['', 's'][instances_running > 2]) + \
                " running!\nI will cause" +\
                " havoc should scans be initiated from more than one!")
        ...
        if log_file_path == None:
            self._config_file.set("log_path", "log" + os.sep + \
                                            "runtime_{0}.log")
        ...
        self._log_file_path = self._program_code_root + os.sep + \
                        log_file_path.format(instances_running)
        ...
        if self._config_file.get("data_root") == None:
            newroot = gtk.FileChooserDialog( \
                            title="Setup: Select experiments root",
        ...
        action_group.add_actions(
            [
                ...
                ("New Experiment",    None,   "New Experiment", None, None,
                ...
                ("Reset Instances Counter", None, "Reset Instances Counter",
                                    None, None, self.reset_Instances_Dialog),
        ...
        self.DMS("Program startup", "Initialising menu", "L",
                                        debug_level='info')
        ...
        #Fixture config GUI
        self.DMS("Program startup", "Initialising fixture GUI", "L",
                                                    debug_level='info')
        ...
        #Grid GUI
        self.DMS("Program startup", "Initialising reGrid GUI", "L",
                                                    debug_level='info')
        self.grid = grid.Grid(self)
        self.vbox.pack_start(self.grid, False, False, 2)

        #Application Settings GUI
        self.DMS("Program startup", "Initialising settings GUI", "L",
                                                    debug_level='info')
        self.app_settings = settings.Config_GUI(self,
                                            conf_file=self._config_file)
        self.vbox.pack_start(self.app_settings, False, False, 2)

        #Setup new Experiment
        self.DMS("Program startup", "Initialising experiment GUI", "L",
                                                    debug_level='info')
        ...
        if DEVELOPMENT:
            self.DMS("Brave one",
                "I hope you are aware that you are running developmental" + \
                " version of " + \
                "Scan-o-Matic.\n\nIt means it can be highly unstable!\n\n" + \
                "If you only want to track new versions look in settings...",
                level="D",
                debug_level="warning")
    ...
    def check_updates(self, widget=None, event=None, data=None):
        ...
        if dev_update or ver_update:
            ...
            self.running_experiments.pack_end(gui_update.Update_Process(self,
                                                                dev_update))
    ...
    def menu_Settings(self, widget=None, event=None, data=None):
        self.DMS('Settings', 'Application Settings Activated', "L",
                                                debug_level='info')
    ...
    def set_installed_scanners(self):
        ...
        for scanner in xrange(\
                    int(self._config_file.get("number_of_scanners"))):
    ...
    def experiment_New_One_Scan_Color(self, widget=None, event=None,
                                            data=None):
    ...
    def experiment_Start_New(self, widget=None, event=None, data=None):
        ...
        if self.experiment_layout._selected_scanner != None:
            ...
            for matrix in self.experiment_layout.plate_matrices:
                if matrix.get_active() >= 0:
                    pinning_matrices.append(self.experiment_layout.\
                        pinning_matrices[matrix.get_active_text()])
                        #get_model()\
                        #[matrix.get_active()][0]])
                else:
            ...
            experiment.Scanning_Experiment(self, self._handle,
                ...
                fixture=self.experiment_layout.fixture.\
                get_active_text().replace(" ", "_"),
    ...
    def update_live_scanners(self):
        ...
        self.DMS('Scanner Resources', 'Live scanners {0}'.format(\
            self._live_scanners),
    ...
    def get_unclaimed_scanners(self):
        ...
        for scanner in self._installed_scanners:
            ...
            lock_file = "{0}{1}_{2}.lock".format( \
                self._program_config_root, os.sep, scanner.replace(" ", "_"))

            self.DMS("Scanner Resources", "Looking for lockfile '{0}'".format(\
    ...
    def set_claim_scanner(self, scanner):
        ...
        try:
            ...
            self.DMS('Scanner Resources',
                "Claimed scanner {0}".format(scanner),
    ...
    def set_unclaim_scanner(self, scanner):
        ...
        if scanner is not None:
            ...
            try:
                ...
                self.DMS('Scanner Resources',
                    "Could not unclaim scanner {0} for unkown reasons".format(\
                    scanner),
                    level="LA", debug_level='warning')
    ...
    #
    def menu_unclaim_scanner_by_force(self, widget=None, event=None,
                                                        data=None):
        ...
        if resp != gtk.RESPONSE_CANCEL:
            ...
            dialog = gtk.MessageDialog(self.window,
                    ...
                    "And make sure no other instance of Scan-o-Matic is" + \
                    " using it!")
            ...
            if resp2 == gtk.RESPONSE_YES:
                ...
                self.DMS("Force Unclaim Scanner",
                        "{0} is now free to use".format(\
    ...
    def ask_Quit(self):
        ...
        if len(self.running_experiments.get_children()) > 1:
            dialog = gtk.MessageDialog(self.window,
                            gtk.DIALOG_DESTROY_WITH_PARENT,
                            gtk.MESSAGE_WARNING, gtk.BUTTONS_NONE,
                            "There are still projects running, " + \
            ...
            if resp == gtk.RESPONSE_YES:
                ...
                children[0].set_text("Aborting running processes, " +\
                    "if scans are running it may take some time. Be patient.")
                self.DMS("Scan-o-Matic",
                    "Shutting down, but processes need to end nicely first",
                    "LA")
    ...
    def auto_reload(self):
        ...
        if not reloaded:
            ...
            self.DMS("Restart",
                "Failed to restart the application, do it manually instead!",
    ...
    def _DMS_tracebacks(self, excType, excValue, traceback):
        ...
        self.DMS("Code Error Encountered",
            "An error in the code was encountered.\n" + \
            "Please close the program as soon as no experiment is running" + \
            " and send " + \
            "the file '{0}' to martin.zackrisson@gu.se".format(
            self._log_file_path),
    ...
    def DMS(self, title, subject, level=None, debug_level='debug'):
        """
            ...
            This function outputs a message comprised of a title and a
            potentially multi-row subject to one or more places.
        ...
        if "L" in level:
            ...
            if debug_level in ['critical', 'error', 'warning', 'info',
                                                            'debug']:

                eval("self._logger." + debug_level)("{0}: {1}".format(
                                            str(title).upper(), subject))

        if "D" in level:

            gtk_debug_levels = {'critical': gtk.MESSAGE_ERROR,
                            'error': gtk.MESSAGE_ERROR,
                            'warning': gtk.MESSAGE_WARNING,
                            'info': gtk.MESSAGE_INFO,
                            'debug': gtk.MESSAGE_INFO}
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(description='This is the main GUI." + \
