...
class Model(new_model_generic.Model):
    ...
    _PRESETS_PROJECT = {
        '_keyMappings': (
            ...
            ('pinningMatrices', 'pinnings'),
            ('localFixture', 'local-fixture'),
            ('lastImage', 'last-image'),
            ('outputDirectory', 'output-path'),
            ('watchPosition', 'watch'),
            ('gridImageIndices', 'grid-indices'),
            ('supressUnwatched', 'supress-unwatched'),
            ('xmlFormat', 'xmlFormat'),
            ('gridCorrection', 'grid-correction')
            ),
        'config-file': None,
        'last-image': None,
        'local-fixture': None,
        'watch': None,
        'supress-unwatched': None,
        'grid-indices': None,
        'grid-correction': None,
        'log_file': '',
        'log_file_dir': '',
        '_output-default': 'analysis',
        'output-path': 'analysis',
        'plates': 0,
        'pinnings': (),
        'pinnings-from-file': (),
        'pinnings-active': 'file',
        'adjustedPriority': None,
    ...
    def projectAnalysisLabel(self):

        return "{0}:{1}>{2}".format(
            os.path.dirname(self['log_file']).split(os.sep)[-1],
            os.path.basename(self['log_file'])[:16],
            self['output-path'])

    def xmlFormat(self):
        return None

    def analysisProjectPrioriy(self):

        if self['adjustedPriority']:
            return self['adjustedPriority']

        return 0

    def analysisKwargs(self):

        kwargs = dict()
        for k, refK in self["_keyMappings"]:
            if self[refK] is not None:
                kwargs[k] = self[refK]
