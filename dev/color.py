...
class Color_Image(object):

    def __init__(self, im_path, parent=None, external_fig=True,
                        pinning_matrix=(12, 8)):
    ...
    def set_deband_image(self, im_norm_low, im_norm_high):
        ...
        norm_profile = self._im_debanded[
                im_norm_low:im_norm_high, :, :].mean(0)
    ...
    def get_gs_im(self, rect=None, use_ref_vector=None):
        ...
        if use_ref_vector is None:
            ...
            if use_ref_vector.upper() == "NEG":
                ...
                n_vector = np.array([f.get_rgb_unit_vector() for f in
                                    self.get_neg_features()])

            elif use_ref_vector.upper() == "POS":

                n_vector = np.array([f.get_rgb_unit_vector() for f in
                                    self.get_pos_features()])

            elif use_ref_vector.upper() == "REF":

                n_vector = np.array([f.get_rgb_unit_vector() for f in
                                    self.get_ref_features()])
    ...
    def get_im_section(self, rect=None, margin=None):
        ...
        if margin:
            ...
            if type(margin) == types.IntType:
                ...
                r_im = im[margin: im.shape[0] - margin,
                        margin: im.shape[1] - margin]
    ...
    def get_plt_free_detect(self):
        ...
        for k, v in self._filter_c_o_m.items():
            ...
            ax.text(v[1], v[0], str(k), bbox=dict(facecolor='white',
                                alpha=0.5))
    ...
    def get_plt_features_detected(self):
        ...
        features_with_filter = [f for f in self._features
                                if f.get_has_filter()]

        plt_grid = self.get_plts_grid(len(features_with_filter),
                                need_even=True)
    ...
    def get_plt_features(self, features=None, f=None, title="",
        ...
        if features is None:
            ...
            features = [(feat.get_identifier(), feat.get_center(),
                feat.get_rgb_str())
        ...
        if quality is not None:
            ...
            ax.text(im.shape[1] * 0.8, im.shape[0] * 0.99,
                "Color quality {0:.2f}".format(quality),
                fontsize='x-small')
    ...
    def get_plt_feature_by_identifier(self, identifier):
        ...
        if f:
            ...
            ax = fig.add_subplot(plts, 1, 1, title='Feature {0}'.format(
                                    f.get_identifier()))
            ...
            if plts == 2:
                ...
                ax = fig.add_subplot(2, 1, 2, title='Filter {0}'.format(
                                    f.get_identifier()))
    ...
    def set_free_detect(self):
        ...
        blobs, qualities, c_o_m, filter_array = \
                im_all_blobs.get_candidate_blob_ranks()
    ...
    def set_grid(self, interactive=True):
        ...
        #im3_grid = g.get_analysis(get_gs_im(im3_cropped), pinning_matrix,
                #use_otsu=True,
        ...
        if interactive:
            ...
            row_move = raw_input(
                "In terms of steps, many rows should rows be moved? ")
            ...
            self.get_plt_grid(grid=self._grid,
                title="Gridding as returned").show()

    def set_grid_move(self, col_move=0, row_move=0):

        self._grid[0] = np.array(r_signal.move_signal([list(self._grid[0])],
                            [col_move], freq_offset=0)[0])

        self._grid[1] = np.array(r_signal.move_signal([list(self._grid[1])],
                            [row_move], freq_offset=0)[0])
        ...
        for r_i, row in enumerate(self._grid[0]):
            ...
            for c_i, column in enumerate(self._grid[1]):
                ...
                if q_inv > 900:
                    ...
                    self.logger.warning('Colony {0}'.format(
                        settings['identifier']) +\
                        ' had bad blob detection quality,' +\
                    ...
                    blob2 = cell.Blob(self._parent, f.get_identifier(),
                            ...
                            image_color_logic=None,
                            center=None, radius=None)

                    q_2 = blob2.get_candidate_blob_ranks()[1][0]

                    self.logger.info('Colony {0}'.format(
                        settings['identifier']) +
            ...
            if verbose:
                ...
                self.logger.info('Got {0:.2f}% of colonies analysed'\
                    .format(100.0 * (r_i + 1) * (c_i + 1) / (len(
                    self._grid[0]) * len(self._grid[1]))))
    ...
    def get_feature_by_identifier(self, identifier):
        ...
        try:
            ...
            ret_val = [f for f in self._features
                    if f.get_identifier() == identifier][0]
    ...
    def get_untrusted_features(self):
        ...
        ret_val = [f for f in self._features if f.get_identifier() in
                            self._untrusted]
    ...
    def get_phenotype_corrected(self, phenotype, coeffs):
        ...
        ret_val = coeffs[0] * phenotype + coeffs[1] * \
                np.exp(phenotype) - coeffs[1]
    ...
    def get_phenotypes(self, length_normed=True, expected_ref_mean=0.5):
        ...
        pos_vector = np.array([get_vector(f) for f in
                        self.get_pos_features()]).mean(0)

        neg_vector = np.array([get_vector(f) for f in
                        self.get_neg_features()]).mean(0)
        ...
        if len(ref_names) > 0:
            ...
            q = 1 / (1 + np.exp(-24 * (0.25 - abs(ref_mean -
                                        expected_ref_mean))))

            coeffs = self.get_phenotype_correction_coeffs(ref_mean,
                                        expected_ref_mean)
...
class Feature():
    ...
    def __init__(self, parent, rect, settings={}):
        ...
        self._parent.logger.debug(
            "FEATURE: starts with settings {0}".format(settings))
        ...
        if self._rect is None and 'rect-size' in settings.keys():
            ...
            self._rect = [[self._center[0] - gcs[0],
                    self._center[0] + gcs[0]],
                    [self._center[1] - gcs[1],
                    self._center[1] + gcs[1]]]
    ...
    def get_rgb_str(self):
        ...
        ret_val = str(map(round, l))[1: -1].replace(".0", "").replace(",", "")
    ...
    def set_rgb_vector(self):
        ...
        if self._filter_array is not None:
            ...
            self._rgb_vector = self._parent.get_im_section(
                                rect=self._rect).mean(0).mean(0)
    ...
    def set_rgb_from_filter(self):
        ...
        self._rgb_vector = im_section[np.where(
                            self._filter_array > 0)].mean(0)
